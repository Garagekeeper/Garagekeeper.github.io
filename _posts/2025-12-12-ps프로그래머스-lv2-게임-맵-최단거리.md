---
title: "[PS][프로그래머스-lv2] 게임 맵 최단거리"
date: 2025-12-12 21:08:01 +0900
description: ""
...
struct Cell
{
    ...
    int _y;
    int _dist;
    Cell(int x, int y, int dist = 1) : _x(x), _y(y), _dist(dist) {}
};
>
...
>
int BFS(Cell start, Cell target)
{
    int n = board.size();
    int m = board[0].size();
...
>
    while (!q.empty())
    {
        ...
        if ((cur._x == target._x) && (cur._y == target._y) )
            return cur._dist;
>
        for (int i = 0; i < 4; i++)
        {
