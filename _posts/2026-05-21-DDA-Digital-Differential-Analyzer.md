...
```
여기서 deltaDIstX를 피타고라스로 구할 수 있는데, x가 1이고 방향 벡터가 있으니 y증가분 (기울기)가 세로 성분의 길이다.
...
```
abs(|ray| / rayDirX) -> abs( 1 / rayDirX)
```
튜토리얼에서는 더욱 간소화해서 위와 같은 형태가 된다. 실제 거리가 아니라 두 값의 비율이 영향을 주기 때문에  로 간단하게 변환된 형태다.
